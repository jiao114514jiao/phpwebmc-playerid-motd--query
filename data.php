...
if (($Info = $Query->GetInfo()) !== true) { //判斷 Query 是否查詢的到
    ...
    $status = "在线"; //伺服器狀態
    $platform = "使用 Ping 无法取得资料"; //伺服器平台 (MINECRAFT or MINECRAFTPE)
    $gametype = "使用 Ping 无法取得资料"; //遊戲類型
    ...
    $host = $host; //伺服器 IP 或網域
    $hostip = "使用 Ping 无法取得资料"; //伺服器主機 IP
    $port = $port; //伺服器端口
    ...
    $software = "使用 Ping 无法取得资料"; //伺服器使用的軟體或核心
